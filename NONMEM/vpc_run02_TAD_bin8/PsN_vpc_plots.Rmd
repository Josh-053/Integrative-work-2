...
#Created 2025-12-02 at 13:41

xpose.runno <- '02.mod'
toolname <- 'vpc'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/home/ali/Documents/GitHub/Integrative-work-2/NONMEM/vpc_run02_TAD_bin8/'
results.directory <- '..'
...
theta.labels <- c('','_','__','___','____','_____')
theta.fixed <- c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)
omega.labels <- c('______','_______')
omega.fixed <- c(FALSE,FALSE)
sigma.labels <- c('________')
sigma.fixed <- c(FALSE)
n.eta <- 2
n.eps <- 1
...
if (is.tte) {
    ...
    plots <- xpose4::xpose.VPC(vpc.info=tool.results.file, vpctab=vpctab, logy = T)
}
print(plots) 
```
