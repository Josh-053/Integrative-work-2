...
ft_all <- bind_rows(
  simulate_group(mod, evobj, MICs, fu, tau, N, egfr_range = c(100,150), group_label = "ICU_eGFR>=96.5"),
  simulate_group(mod, evobj, MICs, fu, tau, N, egfr_range = c(60,90), group_label = "Normal_eGFR<96.5")
)
```
```{r}
ft_summary <- ft_all %>%
...
pta_df <- bind_rows(
  calc_pta(mod, evobj, 2000, MICs, fu, tau, N, c(100,150)),
  calc_pta(mod, evobj, 3000, MICs, fu, tau, N, c(100,150))
)
```
...
ggplot(pta_df, aes(x = MIC, y = PTA, color = factor(Dose))) +
  ...
  scale_color_discrete(name = "Regimen", labels = c("2000 mg q24h", "3000 mg q24h")) +
...
simulate_regimen <- function(dose, MIC, fu, tau, N, egfr_range) {
  idata <- tibble(ID = 1:N, eGFR = runif(N, egfr_range[1], egfr_range[2]))
  evobj <- ev(amt = dose, ii = 24, addl = 14, rate = dose/0.5)
  out <- mrgsim(mod, ev = evobj, idata = idata, end = 363, delta = 0.5)
  df <- as.data.frame(out) %>% left_join(idata, by = "ID")
...
pta2000 <- simulate_regimen(2000, MIC, fu, tau, N, c(100,150))
pta3000 <- simulate_regimen(3000, MIC, fu, tau, N, c(100,150))

pta_all <- bind_rows(pta2000, pta3000)
