...
$PK
TVCL = THETA(1)
  CL = TVCL * EXP(THETA(2) * (eGFR - 96.5)/96.5) * EXP(ETA(1)); THETA(2): describes covariate effect
TVV1 = THETA(3)
  V1 = TVV1 * EXP(ETA(2))
TVV2 = THETA(4)
  V2 = TVV2 * EXP(ETA(3))
...
$TABLE;; output table
ID TIME DV DVID EVID PRED IPRED WRES RES CWRES CL V1 V2 Q K K12 K21 SEX ETHNI BW ALB SCR eGFR AGE NOPRINT ONEHEADER FILE=run02
